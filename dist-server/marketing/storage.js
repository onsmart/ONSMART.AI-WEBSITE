...
export async function uploadMarketingFile(bucket, path, body, contentType) {
    if (!supabaseAdmin)
...
export async function downloadMarketingFile(bucket, path) {
    if (!supabaseAdmin)
        return null;
    const bucketName = bucket === 'pdf' ? BUCKET_PDFS : BUCKET_IMAGES;
...
export async function fetchFileFromUrl(url) {
    try {
        ...
        if (disp && /filename[*]?=(?:UTF-8'')?([^;\s]+)/i.test(disp)) {
            ...
            if (PDF_EXT.test(segment))
                filename = segment;
            else if (IMAGE_EXT.test(segment))
                filename = segment;
        }
        return { buffer: buf, contentType, filename };
    }
