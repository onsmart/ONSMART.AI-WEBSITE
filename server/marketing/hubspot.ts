...
export async function addOrUpdateHubSpotContact(input: HubSpotLeadInput): Promise<{ ok: boolean; error?: string }> {
  if (!token) {
  ...
  if (input.note && contactId) {
    ...
    if (!noteRes.ok) {
      try {
        ...
        if (!r.ok) console.warn('[hubspot] Nota não criada:', await r.text());
      } catch (e) {
