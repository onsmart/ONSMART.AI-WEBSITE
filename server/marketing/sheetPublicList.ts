...
import type { MarketingContent } from './types.js';
...
export async function fetchMarketingSheetJson(
  sheetId: string
): Promise<{ table?: { rows?: { c: Array<{ v?: string }> }[] } }> {
  const url = `https://docs.google.com/spreadsheets/d/${sheetId}/gviz/tq?tqx=out:json&tq=select *`;
  const res = await fetch(url, { headers: { Accept: 'application/json' } });
  if (!res.ok) throw new Error('Falha ao acessar a planilha');
  const text = await res.text();
  const jsonStart = text.indexOf('{');
  const jsonEnd = text.lastIndexOf('}') + 1;
  if (jsonStart < 0 || jsonEnd <= jsonStart) throw new Error('Resposta inválida da planilha');
...
export async function getPublicBlogRowsFromSheet(): Promise<{ rows: MarketingContent[]; total: number }> {
  try {
    ...
    for (const r of tableRows) {
      ...
      if (!title || title === 'TÍTULO') continue;
...
export async function getPublicEbooksRowsFromSheet(): Promise<{ rows: MarketingContent[]; total: number }> {
  try {
