...
import {
  getMarketingUserByEmail,
  getMarketingUserById,
} from './db.js';
import {
  signMarketingAccessToken,
  signMarketingRefreshToken,
  verifyMarketingToken,
  revokeMarketingToken,
} from './jwt.js';
import { setMarketingCookies, clearMarketingCookies } from './cookies.js';
import {
  listMarketingContent,
  getMarketingContentById,
  getMarketingContentBySlug,
  getMarketingContentBySlugAllStatus,
  getMarketingContentIdBySlug,
...
import type { MarketingContentType, MarketingContentStatus } from './types.js';
...
export const marketingRouter = router({
  auth: router({
    ...
    logout: publicProcedure.mutation(async ({ ctx }) => {
      const req = ctx.req as { headers?: { cookie?: string } };
      const cookieHeader = req.headers?.cookie;
      let refreshToken: string | null = null;
      if (cookieHeader) {
        const m = cookieHeader.match(/marketing_refresh=([^;]+)/);
        if (m) refreshToken = decodeURIComponent(m[1]);
      }
      if (refreshToken) {
        const payload = await verifyMarketingToken(refreshToken);
        if (payload) revokeMarketingToken(payload.jti, payload.exp);
      }
      const res = ctx.res;
      if (res && typeof res.setHeader === 'function') {
  ...
  content: router({
    ...
    create: marketingProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const created = await createMarketingContent({
          type: input.type as MarketingContentType,
          status: input.status,
          slug,
          titulo: input.titulo,
    ...
    update: marketingProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!updated) throw new TRPCError({ code: 'NOT_FOUND', message: 'Content not found' });
        return updated;
      }),
    ...
    upload: marketingProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const bucket: BucketKind = input.bucket;
        const { path: uploadedPath, error } = await uploadMarketingFile(
          bucket,
          input.path,
          buf,
          input.contentType
    ...
    sendPdfByEmail: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        let attachment: { buffer: Buffer; contentType: string; filename: string } | null = null;
        if (content.pdf_path) {
        ...
        if (!attachment && content.imagem_url) {
          ...
          if (supabaseMatch) {
            ...
            const bucket: BucketKind = bucketName === 'marketing-pdfs' ? 'pdf' : 'image';
