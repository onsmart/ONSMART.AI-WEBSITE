...
import type { MarketingContentInsert } from './types.js';
...
export async function backfillLegacySiteContents(): Promise<{ inserted: number; skipped: number }> {
  let inserted = 0;
  let skipped = 0;
  for (const row of LEGACY_BLOG) {
    ...
    const input: MarketingContentInsert = {
      type: 'blog_artigos',
      status: 'published',
      slug: row.slug,
      titulo: row.titulo,
