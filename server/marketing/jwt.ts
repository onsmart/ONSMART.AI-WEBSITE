...
export async function signMarketingAccessToken(payload: { sub: string; email: string; jti: string }): Promise<string> {
...
export async function signMarketingRefreshToken(payload: { sub: string; jti: string }): Promise<string> {
  return new jose.SignJWT({ ...payload })
    .setProtectedHeader({ alg: 'HS256' })
    .setSubject(payload.sub)
    .setJti(payload.jti)
...
export async function verifyMarketingToken(token: string): Promise<{ sub: string; email?: string; jti: string; exp: number } | null> {
  try {
...
export function revokeMarketingToken(jti: string, exp: number): void {
  revokeToken(jti, exp);
}
