...
export type CreateContextOptions = { req: { headers: { cookie?: string } }; res: { setHeader: (name: string, value: string | string[]) => void } };

export async function createContext(opts: CreateContextOptions) {
  const { req, res } = opts;
  let marketingUser: { id: string; email: string; name: string | null } | null = null;
