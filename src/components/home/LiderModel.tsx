...
const LiderModel = () => {
  ...
  return (
    <section id="lider-model" className="py-12 sm:py-16 md:py-20 bg-gray-50 dark:bg-gray-900">
      <div className="container mx-auto max-w-5xl px-4 sm:px-6 md:px-8">
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="text-gray-600 dark:text-gray-300 max-w-3xl mx-auto text-base sm:text-lg md:text-xl">
            {t('lider.description')}
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-4 sm:gap-6">
          {(t('lider.pillars', { returnObjects: true }) as Array<{
            ...
            return (
              <div key={index} className="text-center p-4 sm:p-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                <h3 className={`font-bold ${colors[index]} mb-2 sm:mb-3 text-base sm:text-lg`}>{pillar.name}</h3>
                <p className="text-gray-600 dark:text-gray-300 text-sm sm:text-base">{pillar.description}</p>
              </div>
            );
          })}
        </div>
