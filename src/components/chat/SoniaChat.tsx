...
// TEMPORARIAMENTE DESABILITADO - WhatsApp não deve aparecer no deploy (import mantido para evitar erros de compilação)
      ...
      <div className={`bg-white rounded-xl shadow-2xl border border-gray-200 transition-all duration-400 ease-out transform ${
        ...
        <div className="flex items-center justify-between p-3 bg-emerald-500 rounded-t-xl relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 relative z-10">
            {/* TEMPORARIAMENTE DESABILITADO - Botão WhatsApp não deve aparecer no deploy */}
            {false && (
              <Button
                variant="ghost"
                size="sm"
                ...
                className={`h-6 w-6 p-0 rounded-full transition-all duration-300 hover:scale-110 ${
                  chatMode === 'whatsapp' 
                    ? 'text-emerald-300 bg-white/30' 
                    : 'text-white hover:bg-white/20'
                }`}
                title={chatMode === 'whatsapp' ? 'Voltar ao chat' : 'Conectar WhatsApp'}
              >
                <svg 
                  className="h-3 w-3" 
                  fill="currentColor" 
        ...
        {!isMinimized && (
          <>
            {/* TEMPORARIAMENTE DESABILITADO - WhatsAppTab não deve aparecer no deploy */}
            {false && chatMode === 'whatsapp' && (
              <WhatsAppTab 
