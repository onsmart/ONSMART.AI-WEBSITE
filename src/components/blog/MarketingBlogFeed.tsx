 ...
 * site = link para /blog/:slug; linkedin = card abre external_url; youtube = card mostra capa/título do vídeo e abre YouTube.
 */
...
import { FileText, ExternalLink, Calendar, Clock, ArrowRight, Youtube } from 'lucide-react';
...
type ContentRow = {
  ...
  meta?: { video_title?: string; video_thumbnail_url?: string } | null;
};
...
function ArticleCard({
  ...
  const imageUrl = isYoutube && article.meta?.video_thumbnail_url
    ? article.meta.video_thumbnail_url.trim()
    : article.imagem_url?.trim();
  const displayTitle = isYoutube && article.meta?.video_title ? article.meta.video_title : article.titulo;
  ...
  const handleClick = () => {
    ...
    if (isYoutube && article.external_url) {
      window.open(article.external_url!, '_blank', 'noopener,noreferrer');
    }
  };
      ...
      <div className="aspect-[16/9] bg-gradient-to-br from-gray-100 via-gray-200 to-gray-100 dark:from-gray-700 dark:via-gray-800 dark:to-gray-700 relative overflow-hidden">
        {imageUrl && !imageError ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-brand-blue/20 via-blue-500/30 to-brand-blue/20 flex items-center justify-center">
            {isYoutube ? <Youtube className="h-12 w-12 text-brand-blue/50" /> : <FileText className="h-12 w-12 text-brand-blue/50" />}
          </div>
        )}
        <Badge className="absolute top-4 left-4 z-10 bg-gradient-to-r from-brand-blue to-blue-600 text-white shadow-lg border-0 px-3 py-1">
          {isYoutube ? <Youtube className="h-3 w-3 mr-1" /> : <FileText className="h-3 w-3 mr-1" />}
          {isYoutube ? 'Vídeo' : isLinkedIn ? 'LinkedIn' : 'Artigo'}
        </Badge>
      ...
      <CardContent className="pt-0">
        <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400 mb-4 pb-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-1.5">
            ...
            <span>{isYoutube ? 'YouTube' : isLinkedIn ? 'LinkedIn' : 'Artigo'}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Clock className="h-3.5 w-3.5" />
            <span>{isYoutube ? 'Vídeo' : 'Leitura'}</span>
          </div>
        </div>
        {isYoutube ? (
          <Button
          ...
          >
            <Youtube className="h-4 w-4 mr-2 group-hover/btn:translate-x-1 transition-transform" />
            Assistir no YouTube
    ...
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
      {rows.map((article, index) => (
        <ArticleCard
          ...
          isYoutube={article.post_source === 'youtube' && !!article.external_url}
        />
      ))}
    </div>
  );
